...
Provides get_db_connection(), is_coordinate_like(), and _STREET_TYPE_TO_CODE.
Import from here in both blueprints and services — never import from blueprints.db.
"""
import os
import re
...
# Maps user-supplied street type suffix (abbreviation or full name) → GNAF street_type_code
...
def get_db_connection():
    """Create and return a new database connection, or None on failure."""
    try:
...
def is_coordinate_like(value) -> bool:
    """Return True if *value* looks like a lat/lng coordinate."""
    if not value:
        return False
    try:
        num = float(value)
        if '.' in str(value) and (-180 <= num <= 180):
            return True
